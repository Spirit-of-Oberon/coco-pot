...
IMPORT Files, Texts, Sets, CRS, CRT;
...
BEGIN CRS.Error(200+nr, 0(*CRS.pos*))
END SemErr;
...
BEGIN
	...
	WHILE action # NIL DO
		...
		action := action^.next
	END;
	Texts.Append(Files.StdOut, out.buf)
END MeltStates;
...
BEGIN
  ...
  Texts.Open(t, "Scanner.FRM"); Texts.OpenReader(fram, t, 0);
	IF t = NIL THEN
		Texts.WriteString(out, "Scanner.FRM not found"); Texts.WriteLn(out);
		Texts.Append(Files.StdOut, out.buf); HALT(99)
	END;
	Texts.Append(Files.StdOut, out.buf);
	...
	Texts.Create(t, scanner); Texts.Append(t, out.buf);
	Texts.Close(t)
END WriteScanner;
