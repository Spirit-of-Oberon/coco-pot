...
IMPORT CRS, CRT, CRA, CRX, Sets, Texts, Files;
...
BEGIN
  ...
  i := 0;
  WHILE i # len - 1 DO 
		IF s[i] = 22X THEN double := TRUE END;
    INC(i)
	END;
	IF ~ double THEN s[0] := 22X; s[len-1] := 22X END
END FixString;
...
BEGIN
  ...
  IF CRS.errors = 0 THEN
    Texts.WriteString(w, " checking"); Texts.Append(Files.StdOut, w.buf);
    CRT.CompSymbolSets;
