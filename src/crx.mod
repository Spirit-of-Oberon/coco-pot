...
IMPORT Files, Texts, Sets, CRS, CRT, SYSTEM;
...
BEGIN
	...
	Texts.WriteInt(w, n, 0); Texts.WriteLn(w); Texts.Append(Files.StdOut, w.buf);
	HALT(99)
END Restriction;
...
BEGIN
	...
	Texts.Open(t, "Parser.FRM");
	IF t = NIL THEN
		Texts.WriteString(w, "Parser.FRM not found"); Texts.WriteLn(w);
		Texts.Append(Files.StdOut, w.buf); HALT(99)
	END;
  ...
  parser[i] := 0X;
	...
	PutS("  nrSets = "); PutI(symSetSize);  PutS(";$");
