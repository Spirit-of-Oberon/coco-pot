...
IMPORT Files, Texts, Sets;

CONST
	maxSymbols* = 192;	(*max nr of t, nt, and pragmas*)
	maxTerminals* = 128;	(*max nr of terminals*)
	maxNt* = 64;	(*max nr of nonterminals*)
	maxNodes* = 768;	(*max nr of graph nodes*)
	normTrans* = 0; contextTrans* = 1;	(*transition codes*)
	maxSetNr = 64;	(* max. number of symbol sets *)
	maxClasses = 32;	(* max. number of character classes *)
...
BEGIN
	NL; Str("Restriction "); Texts.WriteInt(w, n, 0); NL; Texts.Append(Files.StdOut, w.buf);
	HALT(99)
END Restriction;
...
BEGIN
	...
	NL; Texts.Append(Files.StdOut, w.buf)
END PrintSet;
...
BEGIN
	...
	IF ddt[1] THEN
		...
		WHILE i <= maxSet DO
			...
			INC (i)
		END;
		NL; NL; Texts.Append(Files.StdOut, w.buf)
	END;
END CompSymbolSets;
...
BEGIN
	...
	WHILE sp <= lastNt DO  (*for all nonterminals*)
		GetSym (sp, sn);
		IF sn.struct = 0 THEN
			ok := FALSE; NL; Str("  No production for "); Str(sn.name); Texts.Append(Files.StdOut, w.buf)
		END;
		INC(sp)
	END
